...
export class BriefingsController {
  ...
  @Get(':contactId/brief')
  // @UseGuards(AuthGuard)
  async getBriefing(@Request() req: any, @Param('contactId') contactId: string) {
  ...
  // @UseGuards(AuthGuard)
  async refreshBriefing(@Request() req: any, @Param('contactId') contactId: string) {
    const userId = req.user?.id || 'mock-user-id';
  ...
  // @UseGuards(AuthGuard)
  async refreshBriefingExplicit(@Request() req: any, @Param('contactId') contactId: string) {
    const userId = req.user?.id || 'mock-user-id';
    return this.briefingService.refreshBriefing(contactId, userId);
  }
}
